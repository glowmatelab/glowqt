...
async def rank_cmd(client, message):
    ...
    if not counter_data:
        return await message.reply("❌ Abhi tak koi data record nahi hua!")
    ...
    header = "📊 **Daily Leaderboard**" if rank_type == "daily" else "🏆 **All-Time Leaderboard**"
    leaderboard = f"{header}\n\n"
    for rank, (user_id, user_data) in enumerate(top_20, 1):
        leaderboard += f"#{rank}. {user_data['name']} - {user_data['count']}\n"
    leaderboard += f"\n━━━━━━━━━━━━━━━━━━\n"
    leaderboard += f"📈 **Total Messages ({rank_type}):** `{group_total}`"
...
async def handle_rank_buttons(client, callback_query):
    ...
    try:
        ...
        if "MESSAGE_NOT_MODIFIED" in str(e):
            await callback_query.answer("Aap pehle se isi rank par hain! ✨")
        else:
...
async def unified_tagger(client, message):
    ...
    if cmd == "etag":
        ...
        # mtag / atag / vtag → standard mention style
...
async def stats_cmd(client, message):
    ...
    await message.reply(
        ...
        f"**📈 Message Tracking:**\n"
        f"  💬 Daily Tracked Users: `{daily_total}`\n"
        f"  ⭐ All-Time Tracked Users: `{alltime_total}`\n"
        f"  📅 Last Daily Reset: `{data['message_counter'].get('last_reset', 'N/A')}`\n\n"
...
# --- BOOT ---
# ============================================================
print("🌸 QTTAGbot LOADED! (Full Merge: GM/GA/GN + Emoji Fix + Unified active_chats) 🎀")
app.run()
